...
module.exports = (client) => {
	readdirSync('./phrases/').forEach(dir => {
		...
		for(const file of phrases) {
			const pull = require(`../phrases/${dir}/${file}`);
			...
			if(pull.alisas && Array.isArray(pull)) {pull.aliases.forEach(alias => client.aliases.set(alias, pull.name));}
		}
	});
